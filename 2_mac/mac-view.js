...
(function () {
  ...
  function playBoot() {
    ...
    return bootLines.reduce(function (prev, line) {
      ...
      // Terminal bleibt geöffnet auf dem Schreibtisch
      var dockTerm = document.getElementById("dock-item-terminal");
      if (dockTerm) dockTerm.classList.add("has-window");
      return sleep(500);
    });
  });
  }
  ...
  function askForMusicAndDeckblatt() {
    return new Promise(function (resolve) {
      ...
      function goToDeckblatt() {
        ...
        var idx = path.indexOf("/2_mac");
        var base = idx !== -1 ? path.substring(0, idx) : "/";
        if (!base) base = "/";
        var sep = base.endsWith("/") ? "" : "/";
        var deckblattUrl = window.location.origin + base + sep + "4_lebenslauf/index.html";
        try { sessionStorage.setItem("kioskUsed", "1"); } catch (e) {}
        window.location.href = deckblattUrl;
      }
